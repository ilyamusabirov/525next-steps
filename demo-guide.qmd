...
**What does `materialization: type: table` do?** Bruin wraps your SELECT in `CREATE TABLE raw_reviews AS ...` and runs it against the DuckDB database file (`food.db` on disk). DuckDB reads each parquet file, decompresses the row groups, and writes the result into `food.db` in its own columnar format. After this, `food.db` contains a table called `raw_reviews`. The next asset queries that table, not the parquet files.
...
A view re-reads parquet on every query. That read has two boundaries, each with a different cost:

1. **Parquet → DuckDB** (view executes): DuckDB decompresses row groups and converts them into its internal vectors. Real I/O and CPU work, every time. Not zero-copy.

2. **DuckDB → polars/pandas** (Python asset receives results): DuckDB exports its vectors as Arrow arrays. Polars uses Arrow memory natively, so it wraps the buffers directly: zero-copy. Pandas copies string columns (Arrow stores UTF-8, pandas uses Python objects) but can receive numeric columns zero-copy with `pd.ArrowDtype`.

Zero-copy at boundary 2 does not eliminate the decompression cost at boundary 1. A table pays boundary 1 once. Every downstream asset reads from `food.db` (already in DuckDB format) instead of re-decompressing parquet.
