...
This is **eager**: read everything now, store the result. The alternative is `type: view` (lazy): Bruin runs `CREATE VIEW raw_reviews AS ...`, which saves only the query text. No data is read or stored. Every time a downstream asset queries the view, DuckDB re-opens the parquet files, re-decompresses, and re-converts from scratch.

That re-read is not free. DuckDB's `read_parquet()` uses its own parquet reader: it decompresses and converts into DuckDB's internal vectors on every call. There is no Arrow zero-copy happening here. Zero-copy (handing a memory pointer with no conversion) only applies when DuckDB scans an Arrow table that's already in memory, e.g., `duckdb.from_arrow(pyarrow_table)` in Python. Reading from a parquet file on disk always involves real I/O and CPU work.

For a pipeline, the table materialization makes sense: pay the read cost once in `raw_reviews`, then `monthly_summary` queries the fast on-disk table instead of re-reading parquet.
