...
For each of ~10 million products, count reviews and average rating. DuckDB must build a hash table with one entry per product.
...
Find the top 10 most-reviewed products in each category. This needs two expensive operators back to back: a hash table for GROUP BY, then a window buffer for ROW_NUMBER. Both compete for the same 4 GB memory budget. Still fits, but pushing the limit.
...
::: {.callout-note collapse="true" title="Honest comparison: when does a cluster actually beat a bigger machine?"}

A t3a.2xlarge (8 vCPU, 32 GB RAM, \$0.30/hr) with DuckDB would handle every query in this demo, including the 50M-user GROUP BY, probably faster than the \$0.71/hr EMR cluster. Our 22.5 GB dataset is well within DuckDB's comfort zone. Industry benchmarks show DuckDB handling 1 TB on a single 64 GB machine.

**Where Spark actually wins over a bigger single machine:**

1. **S3 read parallelism beyond one NIC.** DuckDB on one machine gets a fraction of the NIC's bandwidth. Spark on N nodes has N times the network throughput. For scan-heavy queries on hundreds of GB+, this matters.
2. **Dataset size beyond ~1-10 TB.** DuckDB has been tested up to 1 TB comfortably. Beyond 10 TB, single-node becomes impractical regardless of RAM (disk I/O, query time).
3. **Fault tolerance for long-running jobs.** If a DuckDB query fails 2 hours in, you start over. Spark's lineage lets another executor pick up from a failed task.
4. **Concurrent multi-user access.** DuckDB is single-process. Spark can serve multiple users/queries simultaneously via YARN resource management.
5. **MLlib/Streaming ecosystem.** DuckDB is SQL-only. Spark has MLlib, Structured Streaming, GraphX. If your pipeline chains SQL feature engineering into distributed ML training, Spark avoids a tool boundary.
6. **Operators that don't spill.** Both DuckDB and Spark spill to disk when memory is full. But DuckDB's `list()` and `string_agg()` aggregates can't spill, and multiple blocking operators in one query can still OOM. Spark's spill is more battle-tested at scale and pools disk across nodes.

**The real question is not "can it run?" but "at what cost and risk?"** For ad-hoc analytics on data up to a few hundred GB, DuckDB on a single large machine is faster, cheaper, and simpler. The cluster earns its keep at TB+ scale, for long-running production pipelines, or when you need concurrency and fault tolerance.

Sources: [DuckDB Memory Management](https://duckdb.org/2024/07/09/memory-management), [Running DuckDB at 10 TB](https://datamonkeysite.com/2025/10/19/running-duckdb-at-10-tb-scale/), [Should You Ditch Spark?](https://milescole.dev/data-engineering/2024/12/12/Should-You-Ditch-Spark-DuckDB-Polars.html), [Spark Memory: Disk Spill](https://towardsdatascience.com/memory-management-in-apache-spark-disk-spill-59385256b68c/)
:::
...
Same query as DuckDB Query 1: count reviews per category. Only 4 groups, so the distributed shuffle is overkill here.
...
Top 10 most-reviewed products per category. Two expensive operators: GROUP BY builds a distributed hash table, then ROW_NUMBER() runs a window sort on each category partition. 2.7x faster: distributed memory and parallelism win on complex queries.
...
\*DuckDB **can** finish this query on one machine: enable spill-to-disk, increase memory to 12 GB, reduce to 1 thread. It works, but it consumes most of the machine's 16 GB and runs much slower (single-threaded, disk I/O for the spilled hash table). SparkSQL distributes the hash table across nodes: more total memory, more parallelism.
...
Do NOT suppress stderr here: the whole point is to see the error messages.

Two intentional failures to show how to read Spark error messages and find logs:

1. **Column typo** (`review_score` instead of `rating`): Spark catches this at query planning time before scanning any data. The AnalysisException tells you exactly which column is missing. Look at driver stderr, Spark UI SQL tab.
2. **Executor OOM**: force all data to one partition with `REPARTITION(1)`, then `COLLECT_LIST` every review title per user. One executor gets everything, runs out of heap, crashes. Look at Spark UI Stages tab, executor logs.

::: {.callout-tip icon="false" title="Where to find logs"}
