...
**Step 1.** Launch the gateway EC2 instance (from your laptop, inside this repo):

```bash
bash infra/create_gateway.sh
```

The script auto-discovers the AMI, public subnet, and security group, launches a `t3a.xlarge` (16 GB RAM) with the S3 instance profile attached, waits for it to start, and prints the public IP and SSH command.

**Step 2.** Copy the demo scripts to the instance and run the setup:

```bash
scp -i ~/mds/mds-ilya-ec2.pem -r demos/sql-on-cluster ubuntu@<public-ip>:~/sql-on-cluster
ssh -i ~/mds/mds-ilya-ec2.pem ubuntu@<public-ip>
cd sql-on-cluster
bash setup.sh
```
...
**Step 3.** Add the gateway to `~/.ssh/config` (the `create_gateway.sh` output prints the exact block to paste). Then connect VS Code via Remote SSH to `gateway`. Open the `~/sql-on-cluster/` folder. Open `01_duckdb_s3.py`, select the "SQL Demo (uv)" kernel, run the first cell to verify.

See the [Infrastructure page](infra.qmd) for architecture diagrams.
...
Run this from your **laptop** (not the EC2 instance) while transitioning to Part 2. The cluster takes ~8 minutes to reach WAITING state, so launch it now and talk through the architecture while it provisions.
...
The script auto-discovers public subnets, creates a 3-node EMR cluster (1 primary + 2 core, `m6a.xlarge`), and prints the cluster ID.
...
**Step 4.** Get the primary node DNS:
...
**Step 5.** Add the primary node to your SSH config (see [Infrastructure page](infra.qmd#connecting-to-the-cluster) for the full config). Then connect:
...
Switch VS Code Remote SSH to the EMR primary node, or use a terminal with `ssh emr-primary`.
...
scp -o ProxyJump=gateway demos/sql-on-cluster/04_spark_ml.py hadoop@<emr-primary-ip>:~/
scp -o ProxyJump=gateway demos/sql-on-cluster/05_spark_fail.py hadoop@<emr-primary-ip>:~/
```

Or run directly via `spark-submit 03_spark_sql.py` from the primary node after copying.
