...
**What does `materialization: type: table` actually do?** When Bruin sees this, it wraps your SELECT in a `CREATE TABLE raw_reviews AS ...` and runs it against the DuckDB database file (`food.db` on disk). The parquet data gets read into memory, and the result is written into `food.db` as a persistent DuckDB table. After this step, `food.db` contains a real table called `raw_reviews` with all the review rows. The next asset (`monthly_summary`) queries that table, not the parquet files directly.

The alternative would be `type: view`, which saves the query but not the data: every time something reads the view, DuckDB re-reads the parquet files. A table pays the read cost once; a view pays it every time.
