...
**Step 2.** SSH into the instance, clone the repo, and run the setup:
...
**Step 3.** Add the gateway to `~/.ssh/config` (the `create_gateway.sh` output prints the exact block to paste). Then connect VS Code via Remote SSH to `gateway`. Open the `~/525next-steps/demos/sql-on-cluster/` folder. Open `01_duckdb_s3.py`, select the "SQL Demo (uv)" kernel, run the first cell to verify.
