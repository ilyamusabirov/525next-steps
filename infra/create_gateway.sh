...
echo "Step 2: Copy demo scripts and run setup:"
echo "  scp -i ${KEY_PATH} -r demos/sql-on-cluster ubuntu@${PUBLIC_IP}:~/sql-on-cluster"
echo "  ssh -i ${KEY_PATH} ubuntu@${PUBLIC_IP}"
echo "  cd sql-on-cluster && bash setup.sh"
echo ""
